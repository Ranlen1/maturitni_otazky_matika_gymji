...
a střed $S$ úsečky $XX'$ leží na ose. Platí tedy, že $|XS| = |SX'|$. Přímka o se nazývá osa souměrnost
i. Osovou souměrnost s osou $o$ se zapisuje jako $O(o) : X \mapsto X'$ a udává že bod $X'$ je obrazem
bodu $X$.
...
Zobrazení se nazývá shodené, jestliže existuje číslo $k > 0$, že pro každé dvá body $X, Y$ roviny a
jejich orbazy $X'Y'$ platí $|X'Y'| = k \cdot |XY|$. Číslo $k$ se nazývá koeficient podobnosti. Každé
...
Podobnost má také dva typy a to přímou a nepřimou podobnost. Toto rozdělení funguje stejně jako
...
Stejnolehlost $H(S, \kappa)$ určená bodem $S$ a nenulovým realným číslem $\kappa$ je zobrazení v
...
pro $\kappa = 1$. Samoružné přímky jsou všechny přímky procházející středem stejnolehlosti pro
